...
- Audio Routing tab now uses a vertical splitter between Available Audio Sources and Current Routes so both sections stay usable on smaller window heights.
...
### Fixed
- Auto-update “Restart to apply”: wait for the single-instance lock to release before copying and launching the new binary, so the updated app starts instead of exiting as a duplicate instance.
...
### Changed
- CI: Set version from tag before build so release binary matches tag (e.g. v0.3.7 → shows 0.3.7)
- .gitignore: clarify build/ and dist/ are CI-only

## [0.3.4] - 2025-03-17

### Changed
- Version bump to test updater (spawn-helper-then-exit restart flow)

## [0.3.3] - 2025-03-17

### Fixed
- Application menu and autostart desktop entries use full path for Icon= so DE does not fall back to Steam icon
...
### Changed
- Version bump to test in-app update process
